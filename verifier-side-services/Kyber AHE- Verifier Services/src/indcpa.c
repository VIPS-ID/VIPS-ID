...
void indcpa_eval_sub(uint8_t c1[KYBER_INDCPA_BYTES], uint8_t c2[KYBER_INDCPA_BYTES], uint8_t cDiff[KYBER_INDCPA_BYTES]) {
  ...
  polyvec_frombytes(&b1, c1);
  polyvec_frombytes(&b2, c2);

  poly_frombytes(&v1, c1+KYBER_POLYVECCOMPRESSEDBYTES);
  poly_frombytes(&v2, c2+KYBER_POLYVECCOMPRESSEDBYTES);

  poly_sub(&v, &v1, &v2); 
  polyvec_sub(&b, &b1, &b2);

  //polyvec_tobytes(cDiff, &b);
  //poly_tobytes(cDiff+KYBER_POLYVECCOMPRESSEDBYTES, &v);
